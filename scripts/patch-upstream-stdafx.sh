...
# DirectoryLevelStorage::prepareLevel runs a chain of NBT / map-mapping
# parsing on a single thread. If any step hangs, we lose visibility of
# where. Insert app.DebugPrintf checkpoints between the major steps so
# the iOS live log shows progress.
DLSC2="$REPO_ROOT/upstream/Minecraft.World/DirectoryLevelStorage.cpp"
if grep -q 'PREP_LEVEL_CKPT' "$DLSC2"; then
    echo "patch-upstream-stdafx: DirectoryLevelStorage.cpp prepareLevel checkpoints already patched, skipping"
else
python3 - "$DLSC2" <<'PY'
import sys
path = sys.argv[1]
...
for old, new in edits:
    if old in patched and new not in patched:
        ...
        if old not in patched:
            sys.exit(f"patch-upstream-stdafx: anchor not found: {old[:60]!r}")

with open(path, 'w', encoding='utf-8', newline='\n') as f:
    f.write(patched)
print("patch-upstream-stdafx: inserted prepareLevel checkpoints")
PY
fi
