...
edits = [
    ...
    (
        ...
        "void Chunk::rebuild()\n"
        "{\n"
        '\tapp.DebugPrintf("CHUNK_REBUILD_CKPT enter this=%p xyz=%d,%d,%d level=%p lr=%p", this, x, y, z, level, levelRenderer);\n'
        "\tPIXBeginNamedEvent(0,\"Rebuilding chunk %d, %d, %d\", x, y, z);",
    ),
    ...
    (
        ...
        '\tapp.DebugPrintf("CHUNK_REBUILD_CKPT before getGlobalIndexForChunk lr=%p level=%p", levelRenderer, level);\n'
        "\tint lists = levelRenderer->getGlobalIndexForChunk(this->x,this->y,this->z,level) * 2;\n"
        "\tlists += levelRenderer->chunkLists;\n"
        '\tapp.DebugPrintf("CHUNK_REBUILD_CKPT after getGlobalIndexForChunk lists=%d", lists);',
    ),
    ...
    (
        ...
        "\tbyteArray tileArray = byteArray(tileIds, 16 * 16 * Level::maxBuildHeight);\n"
        '\tapp.DebugPrintf("CHUNK_REBUILD_CKPT before getChunkAt level=%p xz=%d,%d", level, x, z);\n'
        "\t{\n"
        "\t\tauto *_lc = level ? level->getChunkAt(x,z) : nullptr;\n"
        '\t\tapp.DebugPrintf("CHUNK_REBUILD_CKPT after getChunkAt lc=%p", _lc);\n'
        "\t\tif (!_lc) {\n"
        '\t\t\tapp.DebugPrintf("CHUNK_REBUILD_CKPT bail: getChunkAt returned null");\n'
        "\t\t\tPIXEndNamedEvent();\n"
        "\t\t\treturn;\n"
        "\t\t}\n"
        "\t\t_lc->getBlockData(tileArray);\n"
        '\t\tapp.DebugPrintf("CHUNK_REBUILD_CKPT after getBlockData");\n'
        "\t}",
    ),
    ...
    (
        ...
        '\tapp.DebugPrintf("CHUNK_REBUILD_CKPT before new Region");\n'
        "\tLevelSource *region = new Region(level, x0 - r, y0 - r, z0 - r, x1 + r, y1 + r, z1 + r, r);\n"
        '\tapp.DebugPrintf("CHUNK_REBUILD_CKPT before new TileRenderer region=%p", region);\n'
        "\tTileRenderer *tileRenderer = new TileRenderer(region, this->x, this->y, this->z, tileIds);\n"
        '\tapp.DebugPrintf("CHUNK_REBUILD_CKPT after new TileRenderer tr=%p", tileRenderer);',
    ),
]
