...
// 2-byte wchar_t replacements. Build is compiled with -fshort-wchar so
// wchar_t is 2 bytes (Win64 parity). Apple's libc <wchar.h> functions
// were compiled against 4-byte wchar_t and would misread our strings.
// All wcs* / wmem* / wcstombs use sites in upstream code go through these.
#ifdef __cplusplus
extern "C++" {
static inline size_t mcle_wcslen(const wchar_t *s) {
    size_t n = 0; if (!s) return 0; while (s[n]) ++n; return n;
}
static inline int mcle_wcscmp(const wchar_t *a, const wchar_t *b) {
    while (*a && *a == *b) { ++a; ++b; }
    return (int)(unsigned)*a - (int)(unsigned)*b;
}
static inline int mcle_wcsncmp(const wchar_t *a, const wchar_t *b, size_t n) {
    while (n && *a && *a == *b) { ++a; ++b; --n; }
    if (!n) return 0;
    return (int)(unsigned)*a - (int)(unsigned)*b;
}
static inline wchar_t *mcle_wcscpy(wchar_t *d, const wchar_t *s) {
    wchar_t *r = d; while ((*d++ = *s++) != 0) {} return r;
}
static inline wchar_t *mcle_wcsncpy(wchar_t *d, const wchar_t *s, size_t n) {
    wchar_t *r = d;
    while (n && (*d = *s) != 0) { ++d; ++s; --n; }
    while (n) { *d++ = 0; --n; }
    return r;
}
static inline wchar_t *mcle_wcscat(wchar_t *d, const wchar_t *s) {
    wchar_t *r = d; while (*d) ++d; while ((*d++ = *s++) != 0) {} return r;
}
static inline const wchar_t *mcle_wcschr(const wchar_t *s, wchar_t c) {
    if (!s) return nullptr;
    while (*s) { if (*s == c) return s; ++s; }
    return c == 0 ? s : nullptr;
}
static inline const wchar_t *mcle_wcsrchr(const wchar_t *s, wchar_t c) {
    const wchar_t *r = nullptr; if (!s) return nullptr;
    while (*s) { if (*s == c) r = s; ++s; }
    if (c == 0) return s;
    return r;
}
static inline const wchar_t *mcle_wcsstr(const wchar_t *hay, const wchar_t *needle) {
...
static inline size_t mcle_wcstombs(char *dst, const wchar_t *src, size_t n) {
    size_t i = 0; if (!src) return 0;
    while (src[i] && i < n) { dst[i] = (char)(src[i] & 0xFF); ++i; }
    if (i < n) dst[i] = 0;
    return i;
}
static inline wchar_t *mcle_wmemcpy(wchar_t *d, const wchar_t *s, size_t n) {
    wchar_t *r = d; while (n--) *d++ = *s++; return r;
}
static inline int mcle_wmemcmp(const wchar_t *a, const wchar_t *b, size_t n) {
    while (n--) { if (*a != *b) return (int)(unsigned)*a - (int)(unsigned)*b; ++a; ++b; }
    return 0;
}
static inline wchar_t *mcle_wmemset(wchar_t *d, wchar_t c, size_t n) {
    wchar_t *r = d; while (n--) *d++ = c; return r;
}
}
#endif

#define wcslen   mcle_wcslen
#define wcscmp   mcle_wcscmp
#define wcsncmp  mcle_wcsncmp
#define wcscpy   mcle_wcscpy
#define wcsncpy  mcle_wcsncpy
#define wcscat   mcle_wcscat
#define wcschr   mcle_wcschr
#define wcsrchr  mcle_wcsrchr
#define wcsstr   mcle_wcsstr
#define wcstombs mcle_wcstombs
#define wmemcpy  mcle_wmemcpy
#define wmemcmp  mcle_wmemcmp
#define wmemset  mcle_wmemset
